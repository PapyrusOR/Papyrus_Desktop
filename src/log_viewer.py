...
class LogViewer:
    ...
    def __init__(self, parent: tk.Misc, logger: PapyrusLogger) -> None:
        self.logger = logger
    ...
    def setup_ui(self) -> None:
        ...
        limit_combo.bind("<<ComboboxSelected>>", lambda _event: self.refresh_logs())

        # 按钮组
        tk.Button(toolbar, text="🔄 刷新", command=self.refresh_logs, bg="#e3f2fd", font=("微软雅黑", 9)).pack(
            side="left", padx=5
        )

        tk.Button(toolbar, text="💾 导出", command=self.export_logs, bg="#fff3e0", font=("微软雅黑", 9)).pack(
            side="left", padx=5
        )

        tk.Button(toolbar, text="🗑️ 清空", command=self.clear_logs, bg="#ffebee", font=("微软雅黑", 9)).pack(
            side="left", padx=5
        )
        ...
        search_entry.bind("<KeyRelease>", lambda _event: self.filter_logs())
    ...
    def refresh_logs(self) -> None:
        ...
        self.status_var.set(
            f"已加载 {len(logs)} 条日志 | 类型: {log_type} | 时间: {datetime.now().strftime('%H:%M:%S')}"
        )
